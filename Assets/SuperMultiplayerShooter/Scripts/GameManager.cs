        ...
        void Update()
        {
            if (!isGameOver)
            {
                ...
                if (playerRankings.Length > 0){
                    isGameOver = true;
                    int score = ourPlayer.playerInstance.otherScore + ourPlayer.playerInstance.kills;
                    string name = ourPlayer.playerInstance.playerName;
                    if (PlayerPrefs.GetString("wallet_address") != null && PlayerPrefs.GetString("wallet_address") != "") {
                        StartCoroutine (saveAmmoArcadeScore(score, name, PlayerPrefs.GetString("wallet_address").ToLower()));
                    }
                }
            }
        }

        private IEnumerator saveAmmoArcadeScore(int score, string name, string address)
        {
            string hash = HashString(address + score.ToString() + "sea" + secretKey);
            string post_url = apiUrl + "saveScore?address=" + address + "&hash=" + hash + "&score=" + score.ToString() + "&name=" + name;
